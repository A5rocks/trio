...
if [ "$CHECK_FORMATTING" = "1" ]; then
    ...
    if [ ${NO_TEST_REQUIREMENTS-0} == 1 ]; then
        ...
        python -m pip install -r test-requirements.txt
        sed -i -e 's/rewrite_asserts(tree, source, strfn, config)/print(ast.dump(tree));rewrite_asserts(tree, source, strfn, config)/' /opt/hostedtoolcache/PyPy/3.10.13/x64/lib/pypy3.10/site-packages/_pytest/assertion/rewrite.py
        flags=""
    fi
